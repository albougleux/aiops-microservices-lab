...
RESULTS_DIR="$PROJECT_ROOT/results/ai-agent-continuous/$TEST_NAME-logs"
...
for N in $(seq 1 "$NUM_RUNS"); do
  ...
  $SSH_CMD "$REMOTE_EXEC 'cd $REMOTE_PROJECT_DIR && docker compose --profile continuous down -v && docker compose down -v && docker compose --profile continuous up -d --build'"

  echo "Waiting 10s for containers to initialize..."
  sleep 10

  echo "Phase 2: Validation..."
  $SSH_CMD "$REMOTE_EXEC 'docker ps --format \"{{.Names}} | {{.Status}}\" | grep continuous'"
  $SSH_CMD "$REMOTE_EXEC 'docker logs ai-agent-continuous --tail 5'"
  ...
  echo "Phase 7: Collecting continuous agent logs and restart count..."
  $SSH_CMD "$REMOTE_EXEC 'docker logs ai-agent-continuous 2>&1'" > "$RESULTS_DIR/continuous-agent-$TEST_NAME-run$N.log"
